...
APK="$(find "$WORKDIR/objectid/src-tauri/gen/android/app/build/outputs/apk" -type f -name '*debug*.apk' | sort | head -n 1)"
if [[ ! -f "$APK" ]]; then
  echo "APK not found under $WORKDIR/objectid/src-tauri/gen/android/app/build/outputs/apk" >&2
  exit 1
fi
